...
object SimpleZIOHttpClient3 extends ZIOAppDefault {
...
object HttpClient3:
  def backendLayer = HttpClientZioBackend.layer()
  def clientLayer = ZLayer(
    for {
      backend <- ZIO.service[HttpClientZioBackend]
    } yield SttpClientInterpreter()
      .toClientThrowErrors(
        Endpoints.getUserEndpoint,
        Some(uri"http://localhost:8080"),
        backend
